...
- **v7 — Qwen3-4B trunk rerun (consistent teacher)**: SFT loss looks clean (~0.6, no collapse). Final pipeline FVE = 0.849 across 18 tags vs v6's 0.874 — 2.5 pp **worse**. RL phase OOMs on a single 32 GB V100 (AV + AV_init + AR = 3 × 4B copies don't fit). Conclusion: trunk upgrade gives no measurable gain on this task; mainline stays on 1.7B.
