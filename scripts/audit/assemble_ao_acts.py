...
def main():
    ...
    for org in ["A", "B", "C"]:
        if bs[org] == 0:
            continue
        h = load_file(str(d / f"acts_ao_{org}_{k}.safetensors"))["h"]
    ...
    save_file({"h": org_acts}, str(d / f"acts_ao_org_{k}.safetensors"))
    base = load_file(str(d / f"acts_ao_base_{k}.safetensors"))["h"]
    assert base.shape[0] == meta["n_global"], f"base {base.shape[0]} vs n_global {meta['n_global']}"
    print(f"[assemble] acts_ao_org_{k} {tuple(org_acts.shape)} ; base {tuple(base.shape)} ✓")
